...
const contractForwarder = require('../contracts/MetaTxForwarder.json');
const contractRecipient = require('../contracts/NFT.json');
...
router.get('/env', async function (req, res) {
  try {
    res.json({ network: mumbai, network_id: 80001 });
  } catch (err) {
...
router.get('/owner', async function (req, res) {
  try {
    var provider = new ethers.providers.JsonRpcProvider(ALCHEMY_MUMBAI_URL);
...
router.get('/metatx/:address/:color', function (req, res) {
  try {
    const web3 = new Web3(new Web3.providers.HttpProvider(ALCHEMY_MUMBAI_URL));
